...
private:
    ...
    //Node* findClosestNodeLast(Node* node, const T& data, Node* last_node) //added to check
    //{
    //    if (node == nullptr)
    //    {
    //        return getNext(last_node); //the data is not in the tree
    //    }
    //    if (compare(node->data, data) == 0) //node->data==data   
    //    {
    //        return node;
    ...
    //    {
    //        last_node = node;
    //        return findClosestNodeLast(node->left, data, last_node);
    //    }
    //    if (compare(node->data, data) < 0) //node->data<data
    //    {
    //        last_node = node;
    //        return findClosestNodeLast(node->right, data, last_node);
    //    }
    //    return nullptr;
    //}
...
public:
    ...
    T& getNextDataInTree(Node* current) //ADDEDTOCHECK
    {
        Node* next_node = getNext(current);
        if (next_node != nullptr)
        {
            return getNodeData(next_node);
        }
        return nullptr;
    }
    ...
    {
        int counter = 0;
        Node* current = this->findClosestNodeBigger(MinId_employee);
        Node* end_node = this->findClosestNodeSmaller(MaxId_employee);
    ...
    Node* findClosestNodeBigger( const T& data) //to check 
    {
        Node* node = this->getRoot();
        Node* last_node = nullptr;
        Node* closest_node = findClosestNodeAuxBigger(node, data, last_node);
        return closest_node;
    }


    Node* findClosestNodeSmaller(const T& data) //to check 
    {
        Node* node = this->getRoot();
        Node* last_node = nullptr;
        Node* closest_node=findClosestNodeAuxSmaller(node, data, last_node);
        return closest_node;
        
    }
   
    Node* findClosestNodeAuxBigger(Node* node, const T& data, Node* last_node) //to check
    {
        
        if (node == nullptr)
        {
            return last_node; //the data is not in the tree
        ...
        {
            last_node = node;
            return findClosestNodeAuxBigger(node->left, data, last_node);
        }
        if (compare(node->data, data) < 0) //node->data<data
        {
            last_node = node;
            return findClosestNodeAuxBigger(node->right, data, last_node);
        }
        return nullptr;
    }

    Node* findClosestNodeAuxSmaller(Node* node, const T& data, Node* last_node) //to check
    {
        ...
        {
            last_node = node;
            return findClosestNodeAuxSmaller(node->left, data, last_node);
        }
        if (compare(node->data, data) < 0) //node->data<data
        {
            last_node = node;
            return findClosestNodeAuxSmaller(node->right, data, last_node);
        }
        return nullptr;
    }

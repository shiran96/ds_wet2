...
StatusType DsManager::DsAddPlayer(int PlayerID, int GroupID, int score)
{
    ...
    try
    {
        shared_ptr<Employee> new_player = new Employee(PlayerID, GroupID, score);
        AllplayersHashTable.insert(new_player); //may throw bad_alloc
        this->AlllevelZeroScoreArr[score]++;
        Company* gr = this->unionFindGroups.getGroup(GroupID);
...
StatusType DsManager::DsRemovePlayer(int PlayerID)
{
    ...
    try
    {
        Node<shared_ptr<Employee>>* player_data = AllplayersHashTable.getPlayerById(PlayerID);
        ...
        Company* gr = this->unionFindGroups.getGroup(group_id);
        if (player_level == 0)
        {
...
StatusType DsManager::DsIncreasePlayerIDLevel(int PlayerID, int LevelIncrease)
{
    ...
    Company* playerGroup=unionFindGroups.getGroup(playerGroupId);
                                                  ...
                                                  int lowerLevel, int higherLevel,double * players)
{
    ...
    {
        Company* group=unionFindGroups.getGroup(GroupID);
        if(lowerLevel<=0) //need to count the players at level zero which aren't in the tree
        {
